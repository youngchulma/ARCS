...
//	private static final int queueSize = 5000; // 入库队列大小
	...
	public static void add(Syslog sysLog){
		try {
			...
			if(log.isErrorEnabled()) log.error("put操作日志到缓存queue异常", e);
...
//					if(log.isErrorEnabled()) log.error("日志队列：超过" + queueSize, e);
//				}
					...
					public void run() {
						...
						while (threadRun) {
							try {
								...
								if(log.isErrorEnabled()) log.error("保存操作日志到数据库异常：" + e.getMessage(), e);
								e.printStackTrace();
							}
						}
					}
