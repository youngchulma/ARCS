...
//			String xxx = (String) dataMap.get("xxx");
			...
			try {
				...
				if(log.isErrorEnabled()) log.error("清理访问日志数据失败：" + e.getMessage(), e);
				e.printStackTrace();
			}
			...
			try {
				...
				if(log.isErrorEnabled()) log.error("清理资源负载日志数据失败：" + e.getMessage(), e);
				e.printStackTrace();
			}
		} catch (Exception e) {
			if(log.isInfoEnabled()) log.info("清理数据异常", e);
		}
	}
