...
public abstract class ToolHttp {
	...
	public static String get(boolean isHttps, String url) {
		...
		try {
			...
			try {
				...
				if(log.isErrorEnabled()) log.error("httpClient.close()异常", e);
			}
		}
		return null;
	}
	...
	public static String post(boolean isHttps, String url, String data, String contentType) {
		...
		try {
			...
			e.printStackTrace();
			if(log.isErrorEnabled()) log.error("连接超时：" + url, e);
		} catch (IOException e) {
			e.printStackTrace();
			if(log.isErrorEnabled()) log.error("IO异常:" + url, e);
		} finally {
			try {
				if (null != httpClient) {
					httpClient.close();
	...
	public static CloseableHttpClient createSSLInsecureClient() {
		...
		try {
			...
			SSLConnectionSocketFactory sslsf = new SSLConnectionSocketFactory(sslContext, org.apache.http.conn.ssl.SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
		...
		*/
		return HttpClients.custom().setSSLSocketFactory(createSSLConnSocketFactory()).build();
	}
	...
	public static String httpRequest(boolean isHttps, String requestUrl, String requestMethod, String outputStr) {
		...
		try {
			...
			if (isHttps) {
				...
				javax.net.ssl.SSLSocketFactory ssf = sslContext.getSocketFactory();// 从上述SSLContext对象中得到SSLSocketFactory对象
	 ...
	 * @return
	 */
	public static String mockLogin(String loginUrl, Map<String, String> loginParam) {
		...
		try {
			...
			loginHP.addHeader("User-Agent", loginParam.get("userAgent"));
			HttpResponse loginHR = client.execute(loginHP);
			HttpEntity loginHE = loginHR.getEntity();
			authmark = EntityUtils.toString(loginHE);
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			try {
	 ...
	 * @return
	 */
	public static String mockPost(String authmark, String dataUrl, Map<String, String> dataParam) {
		CloseableHttpClient client = null;
		...
		try {
			...
			post.addHeader("authmark", authmark);
			post.addHeader("User-Agent", dataParam.get("userAgent"));
			HttpResponse dataHR = client.execute(post);
