					...
					try {
						...
						for (AsyncContext ac : acQueue) {
							...
							try {
								...
								if(log.isErrorEnabled()) log.error("Log4jAsyncWriter IOException 异常!", ex);
							}
						}
					} catch (InterruptedException iex) {
						if(log.isErrorEnabled()) log.error("Log4jAsyncWriter InterruptedException 异常! msgQueue.take()", iex);
					}
				}
			}
		});
