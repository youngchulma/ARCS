...
public abstract class BaseController extends Controller {
	...
	protected boolean authCode() {
		...
		if (null != authCodePram && null != authCodeCookie) {
			...
			try {
				...
				e.printStackTrace();
				if(log.isErrorEnabled()) log.error("验证码处理异常，使用了已失效的cookie验证码", e);
				return false;
			}
		}
		return false;
	 ...
	 * @return
	 */
	public Object getJson(Class<?> dto){
		return JSON.parseObject(HttpKit.readData(getRequest()), dto);
	}
	 ...
	 * @return
	 */
	public List<?> getJsonList(Class<?> dto){
		return JSON.parseArray(HttpKit.readData(getRequest()), dto);
	}
	 ...
	 * @return
	 */
	public Object getJsonByPara(String paramName, Class<?> dto){
		return JSON.parseObject(getPara(paramName), dto);
	}
	 ...
	 * @return
	 */
	public List<?> getJsonListByPara(String paramName, Class<?> dto){
		return JSON.parseArray(getPara(paramName), dto);
	}
